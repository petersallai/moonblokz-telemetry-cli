...
fn parse_set_update_interval(params_str: &str) -> Result<Command> {
    ...
    let start_time_str = get_param(&params, "start_time")
        .ok_or_else(|| anyhow!("Missing start_time parameter"))?;
    let end_time_str = get_param(&params, "end_time")
    ...
    let active_period = active_period_str.parse::<u64>()
        .map_err(|_| anyhow!("Invalid active_period: must be a positive integer"))?;
    let inactive_period = inactive_period_str.parse::<u64>()
